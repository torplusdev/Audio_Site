...
if ( class_exists( 'QiAddonsForElementor_Slider_Shortcode' ) ) {
	class QiAddonsForElementor_Blog_Slider_Shortcode extends QiAddonsForElementor_Slider_Shortcode {
		...
		public function map_shortcode() {
			...
			$this->set_scripts(
				apply_filters( 'qi_addons_for_elementor_filter_blog_slider_register_scripts', array(
					'fslightbox' => array(
						'registered' => true
					)
				))
			...
			$this->set_option(
				array(
					...
					'group'      => esc_html__( 'Layout Spacing', 'qi-addons-for-elementor' ),
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes[] = ( ! empty( $atts['overlay_color'] ) || ! empty( $atts['overlay_hover_color'] ) ) ? 'qodef--overlay' : '';
