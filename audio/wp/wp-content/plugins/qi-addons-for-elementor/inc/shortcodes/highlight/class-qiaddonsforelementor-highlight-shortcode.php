	...
	class QiAddonsForElementor_Highlight_Shortcode extends QiAddonsForElementor_Shortcode {
		...
		private function get_highlight_styles( $atts ) {
			...
			if ( ! empty( $atts['highlight_background_color'] ) ) {
				$styles[] = 'background: linear-gradient(to bottom, transparent ' . ( 0 + (int) $atts['top_offset'] ) . '%, ' . $atts['highlight_background_color'] . ' ' . ( 0 + (int) $atts['top_offset'] ) . '%, ' . $atts['highlight_background_color'] . ' ' . ( 100 - (int) $atts['bottom_offset'] ) . '%, transparent ' . ( 100 - (int) $atts['bottom_offset'] ) . '%)';
			}
