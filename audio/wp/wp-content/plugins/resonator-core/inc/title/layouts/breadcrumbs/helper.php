...
if ( ! function_exists( 'resonator_core_register_breadcrumbs_title_layout' ) ) {
	/**
	 ...
	 * @return array
	 */
	function resonator_core_register_breadcrumbs_title_layout( $layouts ) {
		$layouts['breadcrumbs'] = 'ResonatorCoreBreadcrumbsTitle';
		
		return $layouts;
	}
	
	add_filter( 'resonator_core_filter_register_title_layouts', 'resonator_core_register_breadcrumbs_title_layout');
}

if ( ! function_exists( 'resonator_core_add_breadcrumbs_title_layout_option' ) ) {
	/**
	 ...
	 * @return array
	 */
	function resonator_core_add_breadcrumbs_title_layout_option( $layouts ) {
		$layouts['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'resonator-core' );
		
		return $layouts;
	}
	
	add_filter( 'resonator_core_filter_title_layout_options', 'resonator_core_add_breadcrumbs_title_layout_option' );
}

if ( ! function_exists( 'resonator_core_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs html
	 */
	function resonator_core_breadcrumbs() {
		$page_id = qode_framework_get_page_id();
		
		// Breadcrumbs label
		$labels = apply_filters( 'resonator_core_filter_breadcrumbs_label', array(
			'home'        => esc_html__( 'Home', 'resonator-core' ),
			'tag'         => esc_html__( 'Posts tagged "%s"', 'resonator-core' ),
			'author'      => esc_html__( 'Posted by %s', 'resonator-core' ),
			'search'      => esc_html__( 'Search results for "%s"', 'resonator-core' ),
			'404'         => esc_html__( '404 - Page not found', 'resonator-core' ),
			'query_paged' => esc_html__( '(Page %s)', 'resonator-core' )
		) );
		
		// Breadcrumbs variables
		$settings = apply_filters( 'resonator_core_filter_breadcrumbs_settings', array(
		...
		$breadcrumbs_html = '';
		if ( ! empty( $wrap ) ) {
			$breadcrumbs_html = $settings['wrap_before'] . $wrap . apply_filters( 'resonator_core_filter_breadcrumbs_content', $wrap_child, $settings ) . $settings['wrap_after'];
		}
		
		echo apply_filters( 'resonator_core_filter_breadcrumbs_template', $breadcrumbs_html );
	}
}