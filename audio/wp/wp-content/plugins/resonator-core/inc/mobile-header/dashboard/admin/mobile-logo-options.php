...
if ( ! function_exists( 'resonator_core_add_mobile_logo_options' ) ) {
	/**
	 ...
	 */
	function resonator_core_add_mobile_logo_options( $page, $header_tab ) {
		...
		if ( $page ) {
			...
			$mobile_header_tab = $page->add_tab_element(
				array(
					...
					'description' => esc_html__( 'Set options for mobile headers', 'resonator-core' )
				)
			);
			...
			do_action( 'resonator_core_action_after_mobile_logo_options_map', $page );
		}
	}
	
	add_action( 'resonator_core_action_after_header_logo_options_map', 'resonator_core_add_mobile_logo_options', 10, 2 );
}