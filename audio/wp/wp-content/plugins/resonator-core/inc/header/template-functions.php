...
if ( ! function_exists( 'resonator_core_get_header_logo_image' ) ) {
	/**
	 * This function print header logo image
	 *
	 * @param array $parameters
	 */
	function resonator_core_get_header_logo_image( $parameters = array() ) {
		...
		foreach ( $available_logos as $logo ) {
			...
			$logo_image_id = resonator_core_get_post_value_through_levels( 'qodef_logo_' . $logo );
			
			if ( empty( $logo_image_id ) && ! empty( $header_skin ) ) {
				$logo_image_id = resonator_core_get_post_value_through_levels( 'qodef_logo_main' );
			}
			...
			if ( ! empty( $logo_image_id ) ) {
				$logo_image_attr = array(
					...
					'itemprop' => 'image',
					'alt'      => sprintf( esc_attr__( 'logo %s', 'resonator-core' ), $logo )
				);
		...
		if ( $is_enabled ) {
			echo apply_filters( 'resonator_core_filter_get_header_logo_image', resonator_core_get_template_part( 'header/templates', 'parts/logo', '', $parameters ), $parameters );
...
if ( ! function_exists( 'resonator_core_get_header_widget_area' ) ) {
	/**
	 ...
	 */
	function resonator_core_get_header_widget_area( $header_layout = '', $widget_area = 'one' ) {
		$page_id    = qode_framework_get_page_id();
		$is_enabled = get_post_meta( $page_id, 'qodef_show_header_widget_areas', true ) !== 'no';

		if ( $is_enabled ) {
			$parameters  = apply_filters( 'resonator_core_filter_header_widget_area', array(
				'page_id'             => $page_id,
			...
			resonator_core_template_part( 'header/templates', 'parts/widgets', '', $parameters );
		}
	}
}