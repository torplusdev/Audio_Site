...
if ( ! function_exists( 'resonator_core_add_single_image_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function resonator_core_add_single_image_shortcode( $shortcodes ) {
		$shortcodes[] = 'ResonatorCoreSingleImageShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'resonator_core_filter_register_shortcodes', 'resonator_core_add_single_image_shortcode' );
}

if ( class_exists( 'ResonatorCoreShortcode' ) ) {
	class ResonatorCoreSingleImageShortcode extends ResonatorCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'resonator_core_filter_single_image_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'resonator_core_filter_single_image_extra_options', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( RESONATOR_CORE_SHORTCODES_URL_PATH . '/single-image' );
			$this->set_base( 'resonator_core_single_image' );
			$this->set_name( esc_html__( 'Single Image', 'resonator-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds image element', 'resonator-core' ) );
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'resonator_core_single_image', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return resonator_core_get_template_part( 'shortcodes/single-image', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
