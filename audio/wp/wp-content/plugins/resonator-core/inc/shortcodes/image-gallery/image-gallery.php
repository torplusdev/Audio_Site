...
if ( ! function_exists( 'resonator_core_add_image_gallery_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function resonator_core_add_image_gallery_shortcode( $shortcodes ) {
		$shortcodes[] = 'ResonatorCoreImageGalleryShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'resonator_core_filter_register_shortcodes', 'resonator_core_add_image_gallery_shortcode' );
}

if ( class_exists( 'ResonatorCoreListShortcode' ) ) {
	class ResonatorCoreImageGalleryShortcode extends ResonatorCoreListShortcode {
		
		public function map_shortcode() {
			$this->set_shortcode_path( RESONATOR_CORE_SHORTCODES_URL_PATH . '/image-gallery' );
			$this->set_base( 'resonator_core_image_gallery' );
			$this->set_name( esc_html__( 'Image Gallery', 'resonator-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds image gallery element', 'resonator-core' ) );
			...
			$this->set_option( array(
				...
				'name'       => 'images',
				'multiple'   => 'yes',
				'title'      => esc_html__( 'Images', 'resonator-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'image_size',
			...
			$this->map_list_options( array(
				...
				'group'            => esc_html__( 'Gallery Settings', 'resonator-core' )
			) );
		}

		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'resonator_core_image_gallery', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return resonator_core_get_template_part( 'shortcodes/image-gallery', 'templates/image-gallery', $atts['behavior'], $atts );
		}
