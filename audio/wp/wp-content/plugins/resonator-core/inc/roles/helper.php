...
if ( ! function_exists( 'resonator_core_include_role_custom_fields' ) ) {
	/**
	 * Function that includes role custom fields files
	 */
	function resonator_core_include_role_custom_fields() {
		foreach ( glob( RESONATOR_CORE_INC_PATH . '/roles/*/role-fields.php' ) as $role_fields ) {
			include_once $role_fields;
		}
	}
	
	add_action( 'qode_framework_action_custom_user_fields', 'resonator_core_include_role_custom_fields' );
}

if ( ! function_exists( 'resonator_core_register_role_custom_fields' ) ) {
	/**
	 * Function that registers role custom fields files
	 */
	function resonator_core_register_role_custom_fields() {
		do_action( 'resonator_core_action_register_role_custom_fields' );
	}
	
	add_action( 'qode_framework_action_custom_user_fields', 'resonator_core_register_role_custom_fields', 11 );
}

if ( ! function_exists( 'resonator_core_profile_get_social_params' ) ) {
	/**
	 * Returns owner params
	 */
	function resonator_core_profile_get_social_params( $params ) {
	...
	add_filter( 'resonator_membership_filter_user_params', 'resonator_core_profile_get_social_params' );
}

if ( ! function_exists( 'resonator_core_social_edit_profile_fields' ) ) {
	function resonator_core_social_edit_profile_fields( $page, $params ) {
		$roles_social_scope = apply_filters( 'resonator_core_filter_role_social_array', array( 'administrator', 'author' ));
		$enable = false;
	...
	add_action( 'resonator_membership_action_after_dashboard_edit_profile_fields', 'resonator_core_social_edit_profile_fields', 15, 2 );
}

if ( ! function_exists( 'resonator_core_user_profile_social_update' ) ) {
	function resonator_core_user_profile_social_update( $options, $user_id ) {
	...
	add_action( 'resonator_membership_action_update_user_profile', 'resonator_core_user_profile_social_update', 10, 2 );
}