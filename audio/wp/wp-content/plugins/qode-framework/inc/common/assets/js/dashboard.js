(function ( $ ) {
			...
			tabs.addClass( 'qodef--init' ).tabs(
				{
					activate: function ( event, ui ) {
						// This peace of code is required in order to re init dependency
						if ( typeof qodefDependency.initOptions === 'function' ) {
							qodefDependency.initOptions( ui.newPanel );
						}
	...
	var qodefDependency = {
		...
		initOptions: function ( activePanel ) {
			var $dependencyOptions = $( '.qodef-field-content .qodef-field[data-option-name]' );
			var reInit 			   = false;

			if ( typeof activePanel !== 'undefined' ) {
				$dependencyOptions = activePanel.find( '.qodef-field-content .qodef-field[data-option-name]' );
				reInit             = true;
			}

			if ( $dependencyOptions.length ) {
				qodefDependency.initFields( $dependencyOptions, reInit );
			}
		},
		initMenu: function () {
		...
		initFields: function ( fields, reInit ) {
			fields.each(
				function () {
					var $thisOption = $( this );

					if ( $thisOption.parents( '.qodef-repeater-template' ).length <= 0 ) {
						qodefDependency.initField( $thisOption, reInit );
					}
				}
			);
		},
		initField: function ( thisOption, reInit ) {
			var thisOptionType = thisOption.data( 'option-type' );
			...
			} else if ( 'selectbox' === thisOptionType && reInit ) {
				thisOption.trigger( 'change' );
			}
		},
		...
		qodefSelectBoxDependency: function ( option, reInit ) {
			option.on(
				'change',
				function () {
