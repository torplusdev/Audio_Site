(function ( $ ) {
	...
	$( document ).on(
		'resonator_trigger_get_new_posts',
		function () {
			qodefSwiper.init();
			qodefMagnificPopup.init();
		}
			...
			return {
				on: {
					...
					slideChangeTransitionStart: function () {
						$holder.addClass('qodef-transitioning');

						setTimeout(function() {
							$holder.removeClass('qodef-transitioning');
						}, 500); // animation time
					}
				}
			};
	...
	var qodefAppear = {
		init: function () {
			this.holder = $('.qodef--has-appear:not(.qodef--has-horizontal-appear), .qodef--has-custom-appear, .qodef--has-marker-appear');
...
(function ( $ ) {
	...
	$( document ).ready(
		function () {
			...
			qodef.qodefHoverTrigger.init('.qodef-blog .qodef-e-title-link, .qodef-blog .qodef-e-read-more .qodef-button', 'article');
		}
	);
	...
	$( document ).on(
		...
		function ( e, $holder ) {
			if ( $holder.hasClass( 'qodef-blog' ) ) {
				...
				qodefAudioPlayer.init();
				/* article hovered class on title & read more btn */
				qodef.qodefHoverTrigger.init('.qodef-blog.qodef--list .qodef-e-title-link, .qodef-blog.qodef--list .qodef-e-read-more .qodef-button', 'article');
			}
		}
	);

	/**
	 * Re init media element post formats (video)
	 */
	var qodefReInitMediaElementPostFormats = {
		...
		resize: function ( $holder ) {
			var $mediaElement = $holder.find( '.wp-video-shortcode' ).not( '.mejs-container' );
	...
	var qodefAudioPlayer = {
		init: function () {
			this.holder = $('.qodef-blog-audio-player');
	...
	qodef.qodefAudioPlayer = qodefAudioPlayer;
...
(function ( $ ) {
	...
	$( document ).on(
		...
		function ( e, $holder, nextPage ) {
			...
			qodefPagination.appearAnimationNewPosts();
		}
	);
	...
	var qodefPagination = {
		...
		initLoadMore: function ( $holder ) {
			...
			$items.addClass('qodef--appear-before');
		...
		addPosts: function ( $itemsHolder, newItems, nextPage ) {
			if ( typeof nextPage !== 'undefined' && nextPage !== '' ) {
				...
				qodefPagination.appearAnimationNewPosts( $itemsHolder );
			}
		},
		...
		appearAnimationNewPosts: function ( $holder ) {
			console.log('testing');
			var $items = $holder.find('article:not(.qodef--appear-before)');
