...
if ( ! function_exists( 'resonator_add_rest_api_pagination_global_variables' ) ) {
	/**
	 ...
	 * @return array
	 */
	function resonator_add_rest_api_pagination_global_variables( $global, $namespace ) {
		$global['paginationRestRoute'] = $namespace . '/get-posts';

		return $global;
	}

	add_filter( 'resonator_filter_rest_api_global_variables', 'resonator_add_rest_api_pagination_global_variables', 10, 2 );
}

if ( ! function_exists( 'resonator_add_rest_api_pagination_route' ) ) {
	/**
	 ...
	 * @return array
	 */
	function resonator_add_rest_api_pagination_route( $routes ) {
		$routes['pagination'] = array(
			'route'    => 'get-posts',
			'methods'  => WP_REST_Server::READABLE,
			'callback' => 'resonator_get_new_posts',
			'args'     => array(
				'options' => array(
					'required'          => true,
					...
					'description'       => esc_html__( 'Options data is array with all selected shortcode parameters value', 'resonator' ),
				),
			),
		);
	...
	add_filter( 'resonator_filter_rest_api_routes', 'resonator_add_rest_api_pagination_route' );
}

if ( ! function_exists( 'resonator_get_new_posts' ) ) {
	/**
	 * Function that load new posts for pagination functionality
	 *
	 * @return void
	 */
	function resonator_get_new_posts() {

		if ( ! isset( $_GET ) || empty( $_GET ) ) {
			resonator_get_ajax_status( 'error', esc_html__( 'Get method is invalid', 'resonator' ) );
		} else {
			...
			if ( ! empty( $options ) ) {
				...
				$query_args = resonator_get_query_params( $options );
				...
				echo apply_filters( "resonator_filter_{$get_template_part}", $get_template_part( $module . '/' . $shortcode, 'templates/loop', '', $options ) );
				...
				$data = apply_filters(
					'resonator_filter_pagination_data_return',
					array(
						'html'          => $html,
				...
				resonator_get_ajax_status( 'success', esc_html__( 'Items are loaded', 'resonator' ), $data );
			} else {
				resonator_get_ajax_status( 'error', esc_html__( 'Options are invalid', 'resonator' ) );
			}
		}
	}
}

if ( ! function_exists( 'resonator_get_query_params' ) ) {
	/**
	 ...
	 * @return array
	 */
	function resonator_get_query_params( $params ) {
		...
		return apply_filters( 'resonator_filter_query_params', $args, $params );
	}
}

if ( ! function_exists( 'resonator_get_pagination_data' ) ) {
	/**
	 ...
	 * @return array
	 */
	function resonator_get_pagination_data( $plugin, $module, $shortcode, $post_type, $params ) {
		$data = array();
		...
		if ( ! empty( $post_type ) && ! empty( $params ) ) {
			...
			if ( isset( $params['space'] ) && ! empty( $params['space'] ) ) {
				$params['space_value'] = resonator_get_space_value( $params['space'] );
			}
...
if ( ! function_exists( 'resonator_add_link_pages_after_content' ) ) {
	/**
	 * Function which add pagination for blog single and page
	 */
	function resonator_add_link_pages_after_content() {

		$args_pages = array(
			'before'      => '<div class="qodef-single-links qodef-m"><span class="qodef-m-single-links-title">' . esc_html__( 'Pages: ', 'resonator' ) . '</span>',
			'after'       => '</div>',
			'link_before' => '<span>',
			'link_after'  => '</span>',
			'pagelink'    => '%',
	...
	add_action( 'resonator_action_after_blog_single_content', 'resonator_add_link_pages_after_content' );
	add_action( 'resonator_action_after_page_content', 'resonator_add_link_pages_after_content' );
}
